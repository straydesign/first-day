...
export function LoginModal({ isOpen, onClose, onAuthSuccess, onShowTerms, defaultMode = "login" }: LoginModalProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="w-screen h-screen max-w-none m-0 p-4 md:p-8 bg-[#0F1B3A] border-0 animate-scaleIn shadow-none overflow-y-auto">
          ...
          <form onSubmit={handleSubmit} className="space-y-4 max-w-md mx-auto">
            {!isLogin && (
              <div>
                ...
                <Input id="name" type="text" placeholder="Your name" value={name} onChange={(e) => setName(e.target.value)} disabled={loading} className="bg-[#0F1B3A] border-white/10 text-white placeholder:text-white/50" />
              </div>
            )}
            <div>
              <Label htmlFor="email" className="text-white/80">Email</Label>
              <Input id="email" type="email" placeholder="your@email.com" value={email} onChange={(e) => setEmail(e.target.value)} disabled={loading} className="bg-[#0F1B3A] border-white/10 text-white placeholder:text-white/50" />
            </div>
            <div>
              ...
              <Input id="password" type="password" placeholder={isLogin ? 'Enter password' : 'Create password'} value={password} onChange={(e) => setPassword(e.target.value)} disabled={loading} className="bg-[#0F1B3A] border-white/10 text-white placeholder:text-white/50" />
            </div>
            {!isLogin && (
              <div>
                <Label htmlFor="confirmPassword" className="text-white/80">Confirm Password</Label>
                <Input id="confirmPassword" type="password" placeholder="Re-enter password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} disabled={loading} className="bg-[#0F1B3A] border-white/10 text-white placeholder:text-white/50" />
              </div>
            )}
            {!isLogin && (
