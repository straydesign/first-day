...
import { getFlyTarget, setFlyTarget, subscribeFlyTarget } from "./galleryIntent";
...
// Mid-dark palettes — visible enough to read as walls under point lighting,
// dark enough that brand accents punctuate. Pure black palettes disappear.
const NEUTRAL_DARK = ["#1f1f23", "#26262c", "#2d2d36", "#222229", "#33333d"];
...
const BRAND       = ["#fcd02a", "#fb7025", "#f31b5e", "#3075e1"];
...
export function setRoomView(v: ViewKey) {
  ...
  // Social P3 — any view transition cancels a pending marker fly. The override
  // would otherwise point the camera at a stale world position the moment the
  // user lands in a non-gallery room.
  if (state.view === "gallery" || next !== "gallery") {
    if (getFlyTarget() !== null) setFlyTarget(null);
  }
  state = {
    view: next,
    pulse: state.pulse + 1,
...
export function getRoomTarget(): CameraTarget {
  // Social P3 — while inside the gallery, a marker click installs a flyTarget
  // override that points at an arbitrary world position (the marker's
  // approach pose). The same camera lerp machinery dollies the user there.
  // Outside gallery this override is always null because setRoomView clears
  // it on leave.
  if (state.view === "gallery") {
    const fly = getFlyTarget();
    if (fly) return { position: fly.position, lookAt: fly.lookAt, fov: fly.fov };
  }
...
export function useRoomTarget(): CameraTarget {
  const v = useRoomView();
  // Subscribe to flyTarget so CameraRig re-reads the override the moment a
  // marker click lands. The store also notifies on clear, so leaving gallery
  // (which nulls flyTarget via setRoomView) re-renders the camera back to
  // TARGETS[next-view].
  const fly = useSyncExternalStore(subscribeFlyTarget, getFlyTarget, getFlyTarget);
  if (v === "gallery" && fly) {
    return { position: fly.position, lookAt: fly.lookAt, fov: fly.fov };
  }
  return TARGETS[v];
}
