        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-4xl mx-auto">
          <div className="p-4 md:p-12">
            ...
            <div className="mb-0">
              ...
              <Textarea value={goal} onChange={(e) => { setGoal(e.target.value); setError(null); }} placeholder="Type your goal here..." className="px-5 py-4 bg-white/50 border-2 border-white/10 text-lg focus:border-teal-400 focus:ring-4 focus:ring-teal-100 transition-all resize-none min-h-[80px] md:min-h-[120px]" disabled={isGenerating} autoFocus rows={3} />
            </div>
          </div>
        </motion.div>
      </div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-4xl mx-auto">
          <div className="p-4 md:p-12">
            <div className="mb-4 md:mb-8">
              ...
              <Textarea value={why} onChange={(e) => setWhy(e.target.value)} placeholder="Tell us what motivates you..." className="px-5 py-4 bg-white/50 border-2 border-white/10 text-lg focus:border-teal-400 focus:ring-4 focus:ring-teal-100 transition-all resize-none min-h-[80px] md:min-h-[120px]" disabled={isGenerating} rows={3} />
            </div>
            <div className="mb-4 md:mb-8">
              ...
              <div className="flex flex-col gap-3">
                {[{ value: 'beginner' as const, label: 'Beginner', desc: 'Just starting' }, { value: 'intermediate' as const, label: 'Intermediate', desc: 'Some experience' }, { value: 'advanced' as const, label: 'Advanced', desc: 'Experienced' }].map((level, index) => (
                  <button key={level.value} onClick={() => setExperienceLevel(level.value)} disabled={isGenerating} className={`p-4 clip-tile-${['a','b','c'][index] || 'a'} border-2 transition-all text-left ${experienceLevel === level.value ? 'border-teal-400 bg-[#242450]' : 'border-white/10 bg-white/50 hover:border-white/20'}`}>
              ...
              {showOptional && (
                <div className="mt-4 space-y-4 md:space-y-6">
                  <div>
                    ...
                    <Input type="text" value={priorExperience} onChange={(e) => setPriorExperience(e.target.value)} placeholder="e.g., Took an online course, read a book..." className="px-5 py-4 bg-white/50 border-2 border-white/10 text-lg focus:border-teal-400 focus:ring-4 focus:ring-teal-100 transition-all" disabled={isGenerating} />
                  </div>
                  <div>
                    <label className="block text-sm font-semibold text-white/80 mb-2 md:mb-3">How do you like to learn?</label>
                    <Input type="text" value={preferredTactics} onChange={(e) => setPreferredTactics(e.target.value)} placeholder="e.g., Videos, hands-on practice, reading..." className="px-5 py-4 bg-white/50 border-2 border-white/10 text-lg focus:border-teal-400 focus:ring-4 focus:ring-teal-100 transition-all" disabled={isGenerating} />
                  </div>
                </div>
              )}
            </div>
